...
void generateBlake2(unsigned char *data, int data_length, const unsigned char *key, int key_length,
    ...
    rasta_blake2b_ctx blake_ctx;
    if (rasta_blake2b_init(&blake_ctx, (size_t)hash_len, key, (size_t)key_length)) {
        // something went wrong
        return;
    }

    rasta_blake2b_update(&blake_ctx, data, (size_t)data_length);
    rasta_blake2b_final(&blake_ctx, result);
}
...
static void blake2b_compress(rasta_blake2b_ctx *ctx, int last) {
                       ...
                       const void *key, size_t keylen) // (keylen=0: no key)
{
    ...
    for (i = keylen; i < 128; i++) // zero input block
        ctx->b[i] = 0;
    if (keylen > 0) {
        rasta_blake2b_update(ctx, key, keylen);
...
int rasta_blake2b(void *out, size_t outlen,
    ...
    if (rasta_blake2b_init(&ctx, outlen, key, keylen))
        return -1;
    rasta_blake2b_update(&ctx, in, inlen);
    rasta_blake2b_final(&ctx, out);
